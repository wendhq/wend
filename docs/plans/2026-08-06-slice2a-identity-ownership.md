...
- [ ] **Step 6** — `dotnet test`. Expected **154** (152 + 2 new). The four behaviours land in two
tests: rename/move/delete share one (there is no `GET /api/lists/{id}` endpoint, so "reading" is
not applicable at the API layer), and posting into another user's board gets its own.
...
- [ ] **Step 8** — `dotnet test`. Expected **159** (157 + 2 new): read/edit/delete of another user's
label in one test, and the attach case (in both directions) in another.
...
- [ ] **Step 7** — `dotnet test`. Expected **166** (159 + 6 isolation + 1 from Step 2).
...
Expected: 0 warnings; **170 passed, 0 failed** (166 + 4 new).
...
- `dotnet test` green at **170**; `dotnet build` clean at 0 warnings.
